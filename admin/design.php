	...
	protected function add_section_design_general( $wp_customize ) {
		...
		if ( ! empty( $orders ) ) {
			foreach ( $orders as $order ) {
				...
				$select_orders[ $order ] = sprintf( esc_html( 'Order #%s' ), $order );
			}
		} else {
			$args      = array(
				'post_type'      => 'product',
			...
			if ( $the_query->have_posts() ) {
				while ( $the_query->have_posts() ) {
					...
					$select_orders[ $order_id ] = sprintf( esc_html( 'Order #%s' ), $order_id );
					break;
				}
			}
		}
...
//		) );
	...
	private function add_preview_style( $name, $element, $style, $suffix = '', $echo = true ) {
        ...
        <style type="text/css"
			...
			echo esc_attr( $element  . '{' . ( ( $css_param === '' ) ? '' : ( $style . ':' .  $css_value ) )  . '}') ?></style>
		<?php
		$return = ob_get_clean();
		if ( $echo ) {
